...
> 上一版覆盖到 A35.2；本次更新补充 A36 稳定性收口：页数/字体参数一致性、首翻 warm-up 页数缓存失效与真实 EPUB 回归矩阵。
...
| A33 | EPUB 分页精度：首翻 MeasureCache 两遍 warm + 页底行级填满 + 注释行高单次覆盖 | ✅ 2026-09-12 |
| A34 | EPUB 章末注：p.note/note1 并入注释通道 + 正文 [N] 上标点按弹层 + footnotes 随页下发 | ✅ 2026-09-12 |
| A34.1 | 注释样式可配（蓝灰 0.82 + 三色预设 + 字号滑杆）+ 封面启发式 + Right×缩进修复 | ✅ 2026-09-12 |
| A34.2 | EPUB 页内章节标题不再被「去重标题」剥离（阅读页无独立章节头） | ✅ 2026-09-12 |
| A34.3 | 标题 border-bottom 分割线：CSS 物化 → LayoutItem::Hr → 填充 Rect | ✅ 2026-09-12 |
| A35 | 封面提取兜底链（coverpage meta / cover* 图片 / 封面章首图）+ duokan 画廊分页放大 + 翻页 pageCount 钳制章末兜底 | ✅ 2026-09-12 |
| A35.1 | 剑来封面回归（opf_base_path 时机）+ 画廊高度放宽 + SVG xlink:href 正则兜底 | ✅ 2026-09-12 |
| A35.2 | 普通段落 text-align:right 回落正文排版（装饰诗词页） | ✅ 2026-09-12 |
| A36 | 阅读稳定性收口：结构化页数与字体参数一致、warm-up 页数缓存失效、短章/纯图章/画廊/跨章回归矩阵 | 🔄 进行中 |
