...
| 防泄露修正 | 恢复液态玻璃 + ClipOval 裁切；BF 渐变垫仅菜单态 | **刚改完，待真机**（未 commit） |
...
1. **真机**：阅读外壳（转圈是否恢复、Clip 是否消泄露、渐变垫观感）
2. 通过后 **commit + push** 工作区阅读 chrome 改动
3. 阅读 T1+：形态持久化、设置四页（形态/排版/背景/材质）、字体 sheet、背景图网格接线
4. 壳层候选：首页数据真实化（时长/折线埋点，统一流程后）

---

## 四、工程约束（速查，勿再踩）

- Impeller：**禁止多 Lens / 叠 BackdropFilter**；液态外溢用 **Clip 裁切**，不要改成假玻璃
- 阅读 chrome：BF 渐变垫 **仅菜单态**；常驻大 BF 会卡开书（转圈无日志）
...
**建议重开会话开场**：先读本文件 + `reader-menu-chrome-ia.md`，从「真机外壳 / T1 设置四页」继续。
