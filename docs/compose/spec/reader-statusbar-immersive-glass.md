...
| 底栏展开 | 展开面板宽 **= 收起主胶囊宽 barW（≤208）**，禁止 `clamp(..., available)` 撑满；Solid/玻璃同步 |
| 底栏整体 | 高 **60** / 主胶囊 **208**（原 72/248 偏大）；主段 icon 22、面板 icon 20 |
| 底部渐变 | veil `fogAlphas` 加浓（0.72→0），`stops` 前移；底部 `band` 180/280、`extend` **96** |
